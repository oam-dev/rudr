        ...
        match self.parameter_type {
            ParameterType::Boolean => {
                val.as_bool().ok_or_else(|| {
                    format_err!("expected boolean value for {}", self.name.as_str())
                }).and(Ok(()))
            },
            ...
            ParameterType::Null => {
                ...
                val.as_null().ok_or_else( || {
                    format_err!("expected null value for {}", self.name.as_str())
                })
            }
        }
    }
    ...
    fn get_value(&self, val: Option<&ParameterValue>) -> Result<serde_json::Value, failure::Error> {
        match val {
            ...
            None => {
                ...
                self.default.clone().ok_or(format_err!("No value specified for {}", self.name.as_str()))
            },
        }
    }
}
...
pub fn resolve_params(definition: Vec<Parameter>, values: Vec<ParameterValue>) -> Result<ResolvedVals, ValidationErrors> {
    ...
    definition
        .iter()
        .map(|d| {
            let val = values.iter().find(|e| {
                e.name == d.name
            });
            match d.get_value(val) {
                Ok(resolved_val) => ParameterValue{name: d.name.clone(), value: resolved_val},
                Err(e) => {
                    errors.push(e);
                    ParameterValue{name: d.name.clone(), value: serde_json::Value::Null}
                },
            }
        })
        .for_each(|p| { resolved.insert(p.name, p.value); () });
    if errors.len() > 0 {
