...
impl ManualScaler {
    pub fn from_params(name: String, inst_name: String, comp_name: String, params: ParamMap, owner_ref: OwnerRefs, workload_type: String) -> ManualScaler {
        info!("params: {:?}", &params);
        ManualScaler {
            name: name,
    ...
    fn scale(&self, ns: &str, client: APIClient) -> TraitResult {
        ...
        match self.workload_type.as_str() {
            REPLICATED_SERVICE_NAME => {
                let (req, _) = apps::Deployment::read_namespaced_deployment(self.instance_name.as_str(), ns, Default::default())?;
                let res = client.request(req);
                if res.is_ok() {
                    let original: apps::Deployment = res.unwrap();
                    let dep = self.scale_deployment(original);

                    let (req2, _) = apps::Deployment::replace_namespaced_deployment(self.instance_name.as_str(), ns, &dep, Default::default())?;
            ...
            REPLICATED_TASK_NAME => {
                // Scale jobs
                let (jobreq, _) = batch::Job::read_namespaced_job(self.instance_name.as_str(), ns, Default::default())?;
                let jobres = client.request(jobreq);
                if jobres.is_ok() {
                    let original: batch::Job = jobres.unwrap();
                    let new_job = self.scale_job(original);

                    let (req2, _) = batch::Job::replace_namespaced_job(self.instance_name.as_str(), ns, &new_job, Default::default())?;
                    let res2: Result<serde_json::Value, failure::Error> = client.request(req2);
                    if res2.is_err() {
                        let err = res2.unwrap_err();
                        error!(
