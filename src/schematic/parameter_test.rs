...
fn test_resolve_parameters() {
    ...
    assert_eq!(json!("eliot@example.com"), *res.get("email").expect("email should be set"));
    ...
    assert!(res.is_err());
    assert!(res.unwrap_err().to_string().starts_with("validation failed"));
    ...
    assert_eq!(json!(1912), *res.get("yob").expect("yob should be set to default"));
}